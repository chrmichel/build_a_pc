...
def push_temp(index: str) -> list[str]:
    return ["# push temp " + index, "@5", "D=A", "@" + index, "A=D+A", "D=M"] + push_d()
...
def parse_call(fname: str, nArgs: str, returnlabel: str) -> list[str]:
    answer: list[str] = [" ".join(["# call", fname, nArgs])]
    # save caller frame
...
class Translator:
    ...
    def translate(self, words: list[str]) -> list[str]:
        match words:
            ...
            case [str(cmd), str(segment), str(value)] if cmd in [
                "push",
                "pop",
            ] and value.isnumeric():
            ...
            case [
                ...
                str(funcname),
                str(value),
            ] if cmd == "function" and value.isnumeric():
            ...
            case [
                ...
                str(funcname),
                str(value),
            ] if cmd == "call" and value.isnumeric():
